...
`;
        ...
        function buildTOC(summaryMD) {
            ...
            const sanitizePath = (href) => (href || "").replace(/\.md$/i, "");
            const collectText = (tok) => {
                if (!tok)
                    return "";
            ...
            const readEntry = (item) => {
                const entry = { name: "", path: "", subitems: [] };
                let fallbackName = "";
                const visitTokens = (arr) => {
                    if (!arr)
                        return;
                    for (const tok of arr) {
                        ...
                        switch (tok.type) {
                            case "link":
                                if (!entry.name)
                                    entry.name = (tok.text || "").trim();
                                if (!entry.path)
                                    entry.path = sanitizePath(tok.href);
                                break;
                            case "text":
                            case "codespan":
                            case "strong":
                ...
                visitTokens(item === null || item === void 0 ? void 0 : item.tokens);
                if (!entry.name) {
                    fallbackName = fallbackName || collectText(item);
                    if (!fallbackName && typeof (item === null || item === void 0 ? void 0 : item.text) === "string") {
            ...
            const processList = (items, target) => {
                if (!items || !items.length)
                    return;
                for (const item of items) {
                    const entry = readEntry(item);
                    const childLists = ((item === null || item === void 0 ? void 0 : item.tokens) || []).filter((tok) => (tok === null || tok === void 0 ? void 0 : tok.type) === "list");
                    for (const child of childLists) {
                        processList((child === null || child === void 0 ? void 0 : child.items) || [], entry.subitems);
                    }
            ...
            for (const token of tokens) {
                if ((token === null || token === void 0 ? void 0 : token.type) === "list" && Array.isArray(token.items)) {
                    processList(token.items, root);
                }
            }
            const filterEmpty = (entries) => entries.filter(e => !!e.name || (e.subitems && e.subitems.length)).map(e => ({
                name: e.name,
                path: e.path,
